...
from perpetual_predict.experiment.models import ExperimentResult
...
class ExperimentAnalyzer:
    ...
    async def analyze(self, experiment_id: str) -> ExperimentResult | None:
        ...
        variant_preds = await self.db.get_predictions_by_experiment(
            experiment_id, "variant", evaluated_only=True,
        )
        ...
        variant_return = sum(variant_returns)
        ...
        if sample_size >= experiment.min_samples:
            ...
            if metric == "accuracy":
                ...
                # For sharpe, use return-based t-test as proxy
            ...
            if p_value is not None:
                ...
                if is_significant:
                    if metric == "accuracy":
                        recommended_winner = "variant" if variant_accuracy > control_accuracy else "control"
                    elif metric == "net_return":
                        recommended_winner = "variant" if variant_return > control_return else "control"
                    elif metric == "sharpe":
                        recommended_winner = "variant" if variant_sharpe > control_sharpe else "control"
